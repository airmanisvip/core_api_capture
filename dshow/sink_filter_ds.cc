...
HRESULT CaptureInputPin::GetMediaType(IN int iPosition, OUT CMediaType* pmt)
{
  ...
  switch (iPosition + positionOffset)
  {
    ...
    case 1:
	{
      ...
      pvi->bmiHeader.biWidth = _requestedCapability.width;
      pvi->bmiHeader.biHeight = _requestedCapability.height;
      pvi->bmiHeader.biSizeImage = 2 * _requestedCapability.width * _requestedCapability.height;
    ...
    case 2:
	{
      ...
      pvi->bmiHeader.biWidth = _requestedCapability.width;
      pvi->bmiHeader.biHeight = _requestedCapability.height;
      pvi->bmiHeader.biSizeImage = 3 * _requestedCapability.height * _requestedCapability.width;
    ...
    case 3:
	{
      ...
      pvi->bmiHeader.biWidth = _requestedCapability.width;
      pvi->bmiHeader.biHeight = _requestedCapability.height;
      pvi->bmiHeader.biSizeImage = 2 * _requestedCapability.height * _requestedCapability.width;
...
CaptureInputPin::CheckMediaType(IN const CMediaType* pMediaType)
{
  ...
  if (*formatType == FORMAT_VideoInfo)
  {
    ...
    if (*SubType == MEDIASUBTYPE_I420 && pvi->bmiHeader.biCompression == MAKEFOURCC('I', '4', '2', '0'))
	{
  ...
  if (*formatType == FORMAT_VideoInfo2)
  {
    ...
    if (*SubType == MEDIASUBTYPE_UYVY && pvi->bmiHeader.biCompression == MAKEFOURCC('U', 'Y', 'V', 'Y'))
	{
      _resultingCapability.videoType = VideoType::kUYVY;
      return S_OK;  // This format is acceptable.
    }
...
HRESULT CaptureInputPin::Receive(IN IMediaSample* pIMediaSample)
{
  HRESULT hr = S_OK;
  ...
  if (_threadHandle == NULL)
  {
	...
	struct
	{
		...
		DWORD dwThreadID;
		DWORD dwFlags;
	} threadname_info = { 0x1000, "video capture thread", static_cast<DWORD>(-1), 0 };
...
HRESULT CaptureInputPin::SetMatchingMediaType(const VideoCaptureCapability& capability)
{
      ...
      _captureObserver(captureObserver)
{
  ...
  m_pInput = new CaptureInputPin(inputPinName, this, &m_crtFilter, phr, L"VideoCapture");
...
void CaptureSinkFilter::ProcessCapturedFrame(unsigned char* pBuffer, size_t length, const VideoCaptureCapability& frameInfo)
{
...
STDMETHODIMP CaptureSinkFilter::SetMatchingMediaType(const VideoCaptureCapability& capability)
{
  LockReceive();
  LockFilter();
  HRESULT hr;
